...
def allGP(client, message,redis):
  ...
  if text :
    ...
    if re.search(c.sors,text):
      kb = InlineKeyboardMarkup([[InlineKeyboardButton("قناه السورس 📢", url="t.me/wwwuw")],[InlineKeyboardButton("تواصل السورس 💬", url="t.me/A_5bot")],[InlineKeyboardButton("شروحات السورس 📑", url="t.me/tshaketeam")]])
    ...
    if not redis.sismember("{}Nbot:ReplySend".format(BOT_ID),chatID):
      ...
      if redis.hexists("{}Nbot:{}:PHreplys".format(BOT_ID,chatID),text):
        ...
        Bot("sendphoto",{"chat_id":chatID,"photimport requests
from config import *
...
def Ckuser(message):
  ...
  if response["ok"]:
    ...
    kb = InlineKeyboardMarkup([[InlineKeyboardButton("اضغط للاشتراك ⏺", url="t.me/wwwuw")] ])
    ...
    return Falseo":ID,"reply_to_message_id":message.message_id})
